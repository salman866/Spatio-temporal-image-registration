...
def matchFeatures(gdf_S1, gdf_S2, gps_error_dia, angle_tolerance, dist_tolerance):
    ...
    for i,fd_S1 in tqdm(enumerate(featureDescriptor_S1)):
        ...
        for j,fd_S2 in enumerate(featureDescriptor_S2):
            ...
            if (distance_PS1_PS2 <= gps_error_dia) and (len(srcPoint_feature) == len(dstPoint_feature)):
                ...
                brg_flag = np.any(brg_diff > angle_tolerance)
                dist_tol = np.array(src_dist)*dist_tolerance
...
def rangeThresholdBearing(input_list, bin_length):
...
def rangeThresholdDistance(input_list, bin_length):
    bins = int((max(input_list)-min(input_list))/bin_length)
    # Create a histogram
    hist, bins = np.histogram(input_list, bins=bins, density=True)
    # Find the bin with the highest density
...
def getDistBearingThresholds(distList, brgList, bin_length_dist, bin_length_brg):
    dist_lower_thresh, dist_upper_thresh = rangeThresholdDistance(distList, bin_length_dist)
    brg_lower_thresh, brg_upper_thresh = rangeThresholdBearing(brgList, bin_length_brg)
